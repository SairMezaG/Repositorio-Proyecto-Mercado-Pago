const LoginModel = require('../models/NoSQL/Login'); // Verifica que esta ruta sea correcta
...
const createItem = async (req, res) => {
    try {
        const { body } = req;
        console.log(body);
        const data = await LoginModel.create(body);
...
const updateItem = async (req, res) => {
    try {
        const { id } = req.params;
        const { body } = req;
        const data = await LoginModel.findByIdAndUpdate(id, body, { new: true, runValidators: true });
        if (!data) {
            return res.status(404).send({ message: 'Item no encontrado' });
        }
